...
def compute_features(
    ...
    """
    if ref_asset_ticker not in ohlcv_data:
        raise ValueError(f"Reference asset '{ref_asset_ticker}' not found in OHLCV data. Available assets: {list(ohlcv_data.keys())}")
    
    if ohlcv_data[ref_asset_ticker] is None:
        raise ValueError(f"Reference asset '{ref_asset_ticker}' data is None.")
    
    if ohlcv_data[ref_asset_ticker].empty:
        raise ValueError(f"Reference asset '{ref_asset_ticker}' data is empty.")
